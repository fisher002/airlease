    ...
    <div v-if="showType == false">
      <div class="box-top">
        ...
        <div class="top-right">
          ...
          <div style="width:10px"></div>
          <el-button size="mini" type="danger" :disabled="selectionData.length <= 0" icon="el-icon-delete" @click="toDelete()">删除</el-button>
          <div style="width:10px"></div>
          <el-button
            size="mini"
            type="primary"
...
export default {
  ...
  methods: {
    ...
    toDelete(res) {
      this.ids = []; // 初始化
      if (this.selectionData.length > 0 || res) {
        this.ids.push(res);
        this.selectionData.forEach(e => {
          this.ids.push(e.adminId);
        });
