<template>
  <div class="table-box" v-if="infoData">
    ...
    >
      ...
      <el-table-column prop="leaseStatus" align="center" sortable label="状态" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.leaseStatus == 'editing' ? '编辑中' : '审核中' }}</span>
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="100">
        <template slot-scope="scope">
          <el-button @click="toDetail(scope.row)" type="text" size="small">查看</el-button>
          <el-button v-if="scope.row.leaseStatus == 'editing'" type="text" size="small">编辑</el-button>
          <el-button v-if="scope.row.leaseStatus == 'editing'" @click="toDelete(scope.row)" type="text" size="small">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
