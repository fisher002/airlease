<template>
  <div class="box">
    ...
    <div v-show="showInfo" class="lease-info" v-if="leaseData">
      <div class="info-item" v-for="item in leaseData.data" :key="item.airId">
        <div class="item-top">{{item.airName}}</div>
        <div class="item-bottom">
          <div class="item-number">
            <span>租赁数量: {{item.leaseNumber}}</span>
            <span>总价格: {{item.leasePriceSum}}</span>
            <span :style="{'color': item.leaseStatus == 'editing' ? '#F56C6C' : '#67C23A'}">{{item.leaseStatus == 'editing' ? '未处理' : '已处理'}}</span>
          </div>
          <div class="item-number">
            <span class="time">
              <i class="el-icon-date"></i>{{formatDate(item.leaseStartDate)}}
            </span>
            <span class="time">
              <i class="el-icon-date"></i>{{formatDate(item.leaseEndDate)}}
            </span>
          </div>
        </div>
      </div>
