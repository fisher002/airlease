<template>
  <div class="box">
    ...
    <div v-show="showInfo" class="lease-info" v-loading="loading" v-if="leaseData">
      <span class="lease-review" @click="reGetList()">{{loading ? '加载中...' : '点击重新加载'}}</span>
      <div
        class="info-item"
        v-for="item in leaseData"
        :key="item.airId"
        @click="toDetail('/detaillease',item.leaseId)"
      >
        <div class="item-top">{{item.airName}}</div>
        <div class="item-bottom">
          <div class="item-number">
            <span>租赁数量: {{item.leaseNumber}}</span>
            <span>总价格: {{item.leasePriceSum}}</span>
            <span
              :style="{'color': item.leaseStatus == 'editing' ? '#F56C6C' : '#67C23A'}"
            >{{item.leaseStatus == 'editing' ? '未处理' : '已处理'}}</span>
          </div>
          <div class="item-number">
            <span class="time">
      ...
      <span class="lease-review" @click="toLoadMore()">{{remindText}}</span>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    // 获取我的租赁列表
    getLeaseInfoList(res) {
      if (res === "0" && this.params.pageNumber < this.pages - 1) {
        ...
        res => {
          if (res.data.code == 200) {
            ...
            if (res.data.total <= 10) {
              this.remindText = "没有更多了";
            }
            if (res === "0" && res.data.total > 10) {
              this.leaseData.push(...res.data.data);
              this.remindText = "点击加载更多";
              return;
            }
            this.leaseData = res.data.data;
          }
        },
        res => {
          this.loading = false;
    ...
    toLoadMore() {
      this.remindText = "加载中...";
      if (this.leaseData.length % 10 > 0) {
        this.remindText = "没有更多了";
        return;
      }
      this.getLeaseInfoList("0");
    },
    toDetail(path, id) {
      this.$router.push({
        path: path,
