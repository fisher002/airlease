...
const menus = [
  { label: "首页", path: "/index", isExist: true },
  { label: "关于我们", path: "/detail/aboutUs", isExist: false },
  { label: "空调展示", path: "/detail/airShow", isExist: false },
  { label: "客户案例", path: "/detail/customerCase", isExist: false },
  { label: "服务范围", path: "/detail/service", isExist: false },
  { label: "联系我们", path: "/detail/callUs", isExist: false }
];
export default {
  inject: ["reload"],
  data() {
  ...
  methods: {
    toOtherPath(res) {
      if (!res.isExist) {
        this.$message.error("未完善，正在加紧处理中...");
        return;
      }
